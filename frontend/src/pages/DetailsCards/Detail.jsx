    ...
    <div className='detail-page'>
      ...
      <div className="left">
        ...
        <div className="down-main">
          {product.image &&(
            <img src={product.image} className='down' 
            onClick={()=>setMainImg(product.image)} />
          )}
          {product.sImage&&(
            <img src={product.sImage} className='down' 
            onClick={()=>setMainImg(product.sImage)}/>
          )}
          {product.tImage&&(
            <img src={product.tImage} className='down'
      ...
      <div className="qntity"><button onClick={()=>setQuant(Math.max(1,quant-1))}>-</button>
      <span>{quant}</span>
      <button onClick={()=>setQuant(quant+1)}>+</button>
      </div>
      <div className="btn-row">
        <button className='add-btn' onClick={()=>addToCart(product,quant)}>Add to Cart</button>
      <button className='pay' onClick={()=>navigate("/payment",{state:{product,quant}})}>Buy now</button>
      </div>
